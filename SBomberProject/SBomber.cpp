...
SBomber::BombIterator& SBomber::BombIterator::operator++()
{
    ...
    if (curIndex == -1)
        curIndex = 0;
    for (; curIndex < vecDynamicObj_.size(); curIndex++)
    {
        if (typeid(&vecDynamicObj_[curIndex]) == typeid(Bomb))
        {
            ptr = &vecDynamicObj_[curIndex];
            break;
        }
    }
    if (curIndex == vecDynamicObj_.size())
    {
        curIndex = -1;
        ptr = nullptr;
    }
...
SBomber::BombIterator& SBomber::BombIterator::operator++ (int) // постфиксный инкремент
{
    BombIterator tempBombIterator = *this;
...
SBomber::BombIterator& SBomber::BombIterator::operator-- (int) // постфиксный декремент
{
    BombIterator tempBombIterator = *this;
...
DynamicObject* SBomber::BombIterator::operator*() // операция разыменования итератора
{
    return vecDynamicObj_.at(curIndex);
}
bool SBomber::BombIterator::operator==(BombIterator it) // проверка на лог. равенство итераторов
{
    if (this->curIndex == it.curIndex &&
        this->ptr == it.ptr &&
        this->vecDynamicObj_ == it.vecDynamicObj_)
    {
        return true;
    }
    return false;
